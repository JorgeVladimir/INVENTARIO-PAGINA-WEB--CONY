const GROUP_NAME_OVERRIDES: Record<string, string> = {
  // Mantener este mapa para traducir codigos SAP que no lleguen con nombre.
  '100': 'ACTIVOS FIJOS',
  '101': 'LLANTA-RIN13',
  '102': 'LLANTA-RIN14',
  '103': 'LLANTA-RIN15',
...
export const resolveGroupName = (value: any) => {
  const normalized = normalizeGroupCode(value);
  if (!normalized) return '';

  return GROUP_NAME_OVERRIDES[normalized] || normalized;
};

export const getGroupOverrides = () => ({ ...GROUP_NAME_OVERRIDES });
